...
const UnitManagement = () => {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await tertiaryApi.getUnits();
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5 font-medium">
            Manage academic units (courses), credit hours and prerequisites.
          </p>
        </div>
      ...
      <div className="bg-white border text-center border-slate-200 shadow-sm rounded-2xl overflow-hidden p-16">
         {loading ? (
             <div className="flex items-center justify-center text-slate-400">
               <Loader2 className="animate-spin mr-2" /> Loading units...
             </div>
         ) : units.length === 0 ? (
             <>
               <BookOpen size={40} className="mx-auto text-slate-200 mb-3" />
               <p className="font-semibold text-slate-600 text-lg">No units found</p>
               <p className="text-slate-400 text-sm mt-1">Click "New Unit" to get started.</p>
             </>
         ) : (
            <p className="text-slate-600">You have {units.length} units.</p>
         )}
      </div>
    </div>
  );
