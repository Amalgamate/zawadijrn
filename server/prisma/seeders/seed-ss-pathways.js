...
exports.seedSeniorPathways = seedSeniorPathways;
const PATHWAYS = [
...
const CATEGORIES = {
    CORE: [
        { code: 'CORE', name: 'Core', minSelect: 4, maxSelect: 5 },
    ],
    STEM: [
...
async function seedSeniorPathways(prisma) {
    console.log('\n🧭 Seeding Senior Secondary Pathways & Categories...');
    const pathwayIdByCode = new Map();
    for (const p of PATHWAYS) {
        ...
        pathwayIdByCode.set(row.code, row.id);
    }
    for (const pathwayCode of Object.keys(CATEGORIES)) {
...
//# sourceMappingURL=seed-ss-pathways.js.map